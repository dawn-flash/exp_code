...
'''
'''
大包会干扰之前的getPathDelay,getLinkDelay，所以需要排除大包的影响
'''
from tool import *
...
def plotCorrelation(filename,spn):
    '''
    画出队列长度和排队时延的关系
    :param:sln shared path number
    :return:
    '''
    record1 = {}
    ##初始化record1
    ...
    with open(filename, 'r') as f:
        while True:
            ...
            if line:
                ...
                dest_port = oc[45]
                currentNode = int(namespace.split("/")[2])
                queueSize = int(oc[58].split(":")[1])
                key1 = str(currentNode)
                key2 = str(packet_id)
                if dest_ip == "10.1."+str(2+spn)+".2" and  key1 in record1:
                    if key2 in record1[key1] and action == "-":
                        pair = [time-record1[key1][key2][0],record1[key1][key2][1]]
                        record1[key1][key2] = pair
                        x.append(pair[1])
                        assert (pair[0]>=0)
                        y.append(pair[0])
                    elif key2 not in record1[key1] and action == "+":
                        pair = [time,queueSize-1]
                        record1[key1][key2] = pair
                    else:
                        print("error1")
                elif dest_ip == "10.1."+str(4+spn)+".2" and key1 in record2:
                    if key2 in record2[key1] and action == "-":
                        pair = [time - record2[key1][key2][0],record2[key1][key2][1]]
                        record2[key1][key2] = pair
                        x.append(pair[1])
                        assert (pair[0] >= 0)
                        y.append(pair[0])
                    elif key2 not in record2[key1] and action == "+":
                        pair = [time,queueSize-1]
                        record2[key1][key2] = pair
                    else:
                        print("error2")
                ...
                break
        ##标志为出队列的包，不占队列的位置，但是会影响排队时延

    fig = plt.subplots()
    plt.scatter(x,y)
    plt.ylim([-0.0001,0.005])
    plt.xlabel("queueSize")
    plt.ylabel("queueTime")
    plt.title("queueSize-queueTime Correlation")
    plt.show()
    plt.close()
...
if __name__ == "__main__":
    ...
    # effect1("/media/zongwangz/RealPAN-13438811621/myUbuntu/b2b_improvement/data2/ibp/5/improved_b2b5_0.tr")
    # pass
    # plot_b2bWithLog("/media/zongwangz/RealPAN-13438811621/myUbuntu/b2b_improvement/data2/b2b")
    # plotCorrelation("/media/zongwangz/RealPAN-13438811621/myUbuntu/b2b_improvement/data2/QueueChange.tr",8)