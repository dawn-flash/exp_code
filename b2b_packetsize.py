...
def CalDelayCov(dir):
    ...
    for sPN in sharePathNum:
        ...
        for cnt in range(100):
            ...
            ##下面这段逻辑为了保证当前文件生成完全
            if cnt == 99 and sPN <= 7:
                f1 = data_dir + "/" + "back_to_back" + str(sPN+1) + "_" + str(0) + ".tr"
            elif sPN == 8 and cnt == 99:
                break
            elif cnt < 99:
                f1 = data_dir + "/" + "back_to_back" + str(sPN) + "_" + str(cnt+1) + ".tr"
            print(filename)
...
def CalTrueLinkCov(dir):
    ...
    for sPN in sharePathNum:
        ...
        if os.path.exists(filename1):
            os.remove(filename1)
        for cnt in range(100):
            filename = data_dir + "/" + "back_to_back" + str(sPN) + "_" + str(cnt)
            ##下面这段逻辑保证当前文件被完全利用完毕
            f1 = data_dir + "/" + "linkDelay" + str(sPN) + "_" + str(cnt) + ".tr"
            print(filename)
            while not os.path.exists(f1):
                time.sleep(1500)
...
if __name__ == "__main__":
    ...
    ##使用多线程来同时运行两个函数
    threads = []
    threads.append(threading.Thread(target=CalDelayCov,args=(dir,)))
    threads.append(threading.Thread(target=CalTrueLinkCov,args=(dir,)))
    for t in threads:
        t.start()
